...
def app():
    ...
    st.write('  * A paper deck consisting of 52 cards divided into 4 main suits: HEART :heart:, DIAMOND :diamonds: , CLUBS :clubs: , SPADES :spades:.')
    ...
    st.write('  * Each set includes 13 cards from 2 to 10 with numbers cards and 4 special cards KING, QUEEN, JACK, ACES')
    st.write('#### Poker rule')
    st.write('  * According to Texas holdem, one of the most popular variations of the Poker game, where each player is dealt 2 cards and must combine with the 5 community cards on the table to make a result. Match 5 as strong as you can.')
        ...
        if source_index ==2:
            ...
            # if st.button("Load")         
                if processed:
                    ...
                    st.write(newDict[maxText] + " is the highest occur risk. Let's discover " + newDict[maxText])